...
Pendientes de spec §8: `/tablero` y `/export` (TASK-0029/0034).
...
- React 19 + TypeScript + Vite 8. Router por rol: `/login`, `/entrar` (enlace del asociado),
  ...
  y `/admin/usuarios` (IAM), ambos solo superadmin.
...
- Pantallas: `Login`, `Ops` (tres columnas: requerimientos con "Ofrecer cupo" solo si
  ...
  (override y reset de cola con re-autenticación) y `Usuarios` (alta, rol, placas,
  activar/desactivar).
...
(Chromium) y job `db` (servicio Postgres 16 → `db:migrate` + `db:seed` + `test:db`).
...
Ambientes objetivo: `dev`, `staging` (copia anonimizada del Excel, TASK-0035) y `prod`. Staging nunca
...
| Pantallas                | login, ops, member, hseq, finance, admin (cola y usuarios) | + viewer tablero, parámetros           | TASK-0029, 0041     |
